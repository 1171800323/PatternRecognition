...
TrainSamplePath = 'TrainSamples-300.csv'
TrainLabelPath = 'TrainLabels-300.csv'
TestSamplePath = 'TestSamples-300.csv'  # 测试集样本路径
TestLabelPath = 'TestLabels-300.csv'  # 测试集标签路径
TestPredPath = 'Result.csv'  # 测试集预测结果路径
ModelPath = 'model.pth'  # 模型文件路径
...
def train():
    ...
    dev_dataset = MnistDataset(dev_x, dev_y)
...
def eval_model(data_path, label_path=None):
    ...
    predict = np.empty((len(dataset), 100), dtype=np.float32)
    with torch.no_grad():
        for idx, batch in enumerate(dataloader):
            batch_data = batch['data'].to(device)
            predict[idx * Batch:(idx + 1) *
                    Batch] = model(batch_data).cpu()
